...
carries **no latent-volatility recovery** (A33 σ-corr ≈ −4e-4, at the zero nearly all generators
share — only Chronos-2 pulls meaningfully off it, at 0.05739) and its return tails run slightly thin (A28 kurtosis ratio 1.57 vs the ideal 1.0, A1 kurtosis error
0.368 mid-pack).
...
seasonal-trend decoder is sharpest. **Chronos-2**, the pretrained foundation-model anchor, takes the single
metric its fine-tuned one-step conditional is sharpest on — the **teacher-sigma correlation** (A33
**0.05739**, the best latent-vol recovery of any generator), though even this is ~10× below the 0.6163 floor.
Its weakness is the opposite of a collapsed law: fine-tuned autoregressive rollout **over-disperses**, with a
return distribution ~10× more leptokurtic than the target (A28 kurtosis ratio **0.099**) and a generated
per-step std ~1.5× the real. **SBTS, TimeGAN, COSCI-GAN, TimeVAE, TimeVQVAE and GT-GAN** win no
A-metric outright: SBTS, TimeGAN, TimeVAE and TimeVQVAE are each competent seconds on a handful of axes, while
**GT-GAN** carries the most extreme *systematic* return-law collapse — an over-peaked marginal with the
suite's most extreme kurtosis ratio (A28 0.002659, ~375× more leptokurtic than Heston, vs Chronos-2's
milder 10×), the worst A14 KS (0.3881), a severe A1 kurtosis error (281.8) and a near-separable A18-GRU
(0.4871). (Chronos-2 posts a higher *raw* A1 (33213) and log-return-histogram MSE, but those are driven by
its single seed-3 multiplicative runaway rather than a systematic collapse across seeds.)
...
samples. **TimeVQVAE**, **COSCI-GAN**, **GT-GAN** and **Chronos-2** win no B plot; COSCI-GAN posts the worst
QQ MSE (8.25e-04), while **Chronos-2** posts the **worst log-return-histogram MSE of any method** (20704 vs
GT-GAN's 2160) — the curve-space image of its over-dispersed, heavy-tailed rollout (and, at 9718, the
second-worst rolling-vol MSE behind TimeVAE). COSCI-GAN's near-Gaussian marginal matches the low-order
*scalar* moments (A5) but not the full-density *curves*.
...
CRPS-specific and does not carry to point-wise MAE/RMSE. **Chronos-2** also edges the RW at both horizons
(3.719 / 5.218), but only marginally: its fine-tuned rollout is locally so well-calibrated that the
anchored ensemble mean essentially reproduces the random walk (see
[`results/Heston/Chronos2/path_shadowing/`](results/Heston/Chronos2/path_shadowing/)).
...
#### Chronos-2
![Chronos-2 diagnostics](results/Heston/Chronos2/plots/heston_diagnostics.png)
...
GAN, Diffusion, VAE, Schrödinger Bridge, Fourier Flow, Foundation Model). Each row links the
...
<tr><td colspan="7"><strong>🟫 Foundation Model</strong></td></tr>
<tr>
