...
<p align="center">
  ...
  <img alt="Generators: 13" src="https://img.shields.io/badge/generators-13-orange.svg">
...
same harness and appears in every table below as **Deep-MKV-TS**; the twelve
other methods are the competing generative families it is compared against. See [Citation](#citation) for how to cite this work.
...
TimeVAE 1.** TimeGAN, COSCI-GAN, GT-GAN, CSDI, TimeMoDE and TimeVQVAE win none outright. (36 = the 34 metrics with A18 and
...
A21/A22 ACF-lag averages, A31 rolling-vol KS **0.01375**, A32 vol-of-vol). Its losses are structural: the two
Heston-spec rows (A33/A34, latent variance unrecoverable from prices), the lag-1 ACF pair (A23/A24, where
Diffusion-TS's decoder is sharpest), the higher-moment rows (A15 skewness, A28 kurtosis ratio) TimeDiT now
owns, and the MMD/adversarial axes LS4 and Deep-MKV-TS hold.
...
three taken directly off SBTS. That cluster is exactly what the model is built to get right: the
Guyon-Lekeufack reference SDE is calibrated once and never trained, so the conditional mean level is pinned by
construction and only an entropy-penalised volatility correction is learned on top. One of the eight wins
(A11) sits **below the Perfect-Recovery Floor**, which flags an under-dispersed sample rather than
superiority, corroborated by its weak tail rows (A5 5.110 vs floor 0.5266, A28 1.182 vs floor 1.006). It
previously also held A18-MLP; SBBTS has since taken that row, but see the SBBTS paragraph below, A18 is
saturated and the row is not evidence either way. Read the
per-method write-up in [`results/Heston/Deep-MKV-TS/`](results/Heston/Deep-MKV-TS/) before quoting the count.
...
A19-MLP **0.05006**, at or under the finite-sample floor).
...
Its genuine strength is the **path-shadowing** section, where it is at the floor on both horizons. Read
[`results/Heston/SBBTS/`](results/Heston/SBBTS/) before quoting the count.

The remaining families defend narrow niches.
**Fourier Flow** takes the **return-std error** (A26 **0.004832**) and the **teacher-sigma RMSE**
...
floor). **TimeDiT**, the benchmark's largest _dense_ generator (32.46 M), is the suite's best **higher-moment**
matcher, winning the **skewness error** (A15 **0.01515**) and the **kurtosis ratio** (A28 **0.9925**,
essentially on the 1.006 finite-sample floor). **CSDI, TimeMoDE, TimeGAN, COSCI-GAN, TimeVQVAE and GT-GAN**
win no A-metric outright; **GT-GAN** carries the most extreme *systematic* return-law collapse, an over-peaked
...
first is the honest signature of a 47 330-parameter model whose drift is frozen: it cannot chase curve detail
the way a high-capacity sampler can, but it also never collapses. Its worst ratio to the floor is tail
survival (**49.8×**), the same tail-mass deficit visible in its A-table kurtosis rows. **LS4** slips to third
on marginal shape but keeps the one row it owns, its grid_tvd path-cloud (**2.772%**) still edges Deep-MKV-TS
(3.575%) and SBTS (3.809%). **Fourier Flow** and **TimeDiT** form the next tier, FF edges TimeDiT on the
...
→ [`results/Heston/Deep-MKV-TS/`](results/Heston/Deep-MKV-TS/), Deep-MKV-TS metrics, diagnostics, PS-MC
...
which is now second at both (2.704 / 3.763). The margins are small, 0.008 and 0.005, i.e. about 2× and 1× the
cross-seed std, so H=32 is a genuine separation and H=64 is a photo finish. **CSDI** and **Diffusion-TS**
follow within ~0.8% at H=32 (2.718 / 2.717), and CSDI is third at H=64 (3.776). Every method except
**COSCI-GAN** (4.657 / 5.789) and **TimeVAE** (3.912 / 5.670) beats the RW baseline (3.738 / 5.246) at both
horizons, so the generated paths carry real predictive structure beyond the marginal variance; the two
exceptions overshoot the random walk because their samples are over-dispersed (COSCI-GAN) or collapsed
(TimeVAE). Notably **GT-GAN beats RW on CRPS at both horizons** (3.551 / 4.996) despite its collapsed,
over-peaked return law: price anchoring plus K=77 neighbour averaging launders its spiky returns into a
well-calibrated ensemble spread, the gain is CRPS-specific and does not carry to point-wise MAE/RMSE.

> ⚠️ **Both leaders sit below the Perfect-Recovery Floor** (2.721 / 3.788): Deep-MKV-TS at 2.696 / 3.758 and
> LS4 at 2.704 / 3.763. The floor is an independent *true Heston draw* scored through the same harness, and a
> generator cannot legitimately out-forecast the true process. Scoring under it diagnoses an **under-dispersed
> pool**, whose over-shrunk ensemble mean is rewarded by CRPS on a near-martingale target, not a modelling
> win. For Deep-MKV-TS the same deficit shows up independently in the A table (A5 5.110 vs floor 0.5266, A28
> 1.182 vs floor 1.006). Treat the PS-MC ranking as "which pool suits retrieval best", not "which generator is
> most faithful", and read
> [`results/Heston/Deep-MKV-TS/path_shadowing/`](results/Heston/Deep-MKV-TS/path_shadowing/) for the full
> caveat.
...
signature, not a forecasting miracle, see the PS-MC caveat above.) See [`methods/Chronos2/`](methods/Chronos2/),
...
#### Deep-MKV-TS
![Deep-MKV-TS diagnostics](results/Heston/Deep-MKV-TS/plots/heston_diagnostics.png)
...
GAN, Diffusion, VAE, Schrödinger Bridge, Fourier Flow, McKean-Vlasov). Each row links the
...
<tr><td colspan="7"><strong>🟩 McKean-Vlasov</strong></td></tr>
<tr>
<td><a href="methods/Deep-MKV-TS/">Deep-MKV-TS</a></td>
<td>Deep McKean-Vlasov Time Series <sub>(this repo's companion paper, title pending)</sub></td>
<td>El Boustany, Pham, Alouadi, Mekkaoui, Hafsi, Basséras</td><td>2026</td><td>ICAIF 2026, Milan</td>
<td><a href="methods/Deep-MKV-TS/code/reference/">vendored (see code README)</a></td>
<td><a href="methods/Deep-MKV-TS/paper_reimplementation/Deep-MKV-TS_ICAIF.pdf">PDF</a></td>
</tr>
...
→ [`results/Heston/Deep-MKV-TS/README.md`](results/Heston/Deep-MKV-TS/README.md)
...
# 2k. Train Deep-MKV-TS (5 seeds, 2 A100 GPUs, GRU vol-correction over a frozen
#     Guyon-Lekeufack reference, 3000 steps reported at step 2500, ~24-33 min/seed)
bash methods/Deep-MKV-TS/paper_reimplementation/metric/run_reproduction.sh   # seeds 0,1,3,4
bash methods/Deep-MKV-TS/paper_reimplementation/metric/run_seed2.sh          # seed 2
# then everything downstream (export + metrics + plots + PS-MC) in one command:
bash methods/Deep-MKV-TS/code/run_benchmark_pipeline.sh
...
CUDA_VISIBLE_DEVICES=0 python metrics/compute_all.py --method Deep-MKV-TS --dataset Heston
