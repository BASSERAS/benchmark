...
share — TimeVAE pulls furthest off it at 0.02254, still ~27× below the 0.6163 floor) and its return tails run slightly thin (A28 kurtosis ratio 1.57 vs the ideal 1.0, A1 kurtosis error
0.368 mid-pack).
...
seasonal-trend decoder is sharpest. **TimeVAE** takes the single metric its variational latent is sharpest
on — the **teacher-sigma correlation** (A33 **0.02254**, the best latent-vol recovery of any generator),
though even this is ~27× below the 0.6163 floor. **SBTS, TimeGAN, COSCI-GAN, TimeVQVAE and GT-GAN** win no
...
suite's most extreme kurtosis ratio (A28 0.002659, ~375× more leptokurtic than Heston), the worst A14 KS
(0.3881), a severe A1 kurtosis error (281.8) and a near-separable A18-GRU (0.4871).
...
samples. **TimeVQVAE**, **COSCI-GAN** and **GT-GAN** win no B plot; COSCI-GAN posts the worst
QQ MSE (8.25e-04), while **GT-GAN** posts the worst log-return-histogram MSE of any generator (2160).
COSCI-GAN's near-Gaussian marginal matches the low-order
