...
# ── TF1 compat shim (mirrors timegan.py) ──
import tensorflow.compat.v1 as _tf1_v1
_tf1_v1.disable_eager_execution()
_tf1_v1.disable_v2_behavior()
for _n in ['reset_default_graph','placeholder','Session','set_random_seed',
           'global_variables_initializer','variable_scope','get_variable',
           'get_collection','GraphKeys','AUTO_REUSE','all_variables']:
    if not hasattr(tf, _n): setattr(tf, _n, getattr(_tf1_v1, _n))
if not hasattr(tf.nn, 'dynamic_rnn'): tf.nn.dynamic_rnn = _tf1_v1.nn.dynamic_rnn
if not hasattr(tf.nn, 'rnn_cell'): tf.nn.rnn_cell = _tf1_v1.nn.rnn_cell
if not hasattr(tf, 'losses'): tf.losses = _tf1_v1.losses

# tf.contrib.layers.fully_connected compat
if not hasattr(tf, 'contrib') or not hasattr(tf.contrib, 'layers') or not hasattr(tf.contrib.layers, 'fully_connected'):
    if not hasattr(tf, 'contrib'): tf.contrib = type('contrib',(),{})()
    if not hasattr(tf.contrib, 'layers'): tf.contrib.layers = type('layers',(),{})()
    ...
    tf.contrib.layers.fully_connected = _fc
