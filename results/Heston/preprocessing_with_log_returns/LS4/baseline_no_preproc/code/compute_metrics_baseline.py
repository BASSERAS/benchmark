...
Identical to ``LS4/code/compute_metrics_logret.py`` in every computation — it
...
results / plots) instead of the with-preproc ``LS4/`` tree.
...
HERE = os.path.dirname(os.path.abspath(__file__))              # .../LS4/baseline_no_preproc/code
BASE_DIR = os.path.dirname(HERE)                               # .../LS4/baseline_no_preproc
LS4_DIR = os.path.dirname(BASE_DIR)                            # .../LS4
# BENCH_ROOT = 4 dirnames up from LS4_DIR (LS4 -> preprocessing_with_log_returns -> Heston -> results -> benchmark)
...
def load_generated(seed):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}. Run train_ls4_raw.py --seed {seed} first.")
    return np.load(path)
