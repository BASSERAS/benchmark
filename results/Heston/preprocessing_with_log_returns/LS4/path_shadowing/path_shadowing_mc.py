...
LS4 + SBTS log-return preprocessing.

Differs from the original methods/LS4 quick eval (8192-path bank, K=77):
this uses the FULL protocol committed in GUIDELINE.md §9:
    ...
    alignment = additive-endpoint in log == multiplicative price anchoring
                (exactly what path_shadowing.ps_mc_retrieve already does)
    score     = CRPS (energy score), lower better

The PS math (65D murex embedding, retrieval, Gaussian bandwidth, CRPS) is
REUSED verbatim from methods/LS4/path_shadowing/path_shadowing.py — this file
only adds the 1M-bank generator (LS4 prior sampling + SBTS inverse) and the
driver. Nothing under methods/ is modified.

Bank build:
  load results/.../LS4/weights/seed_{i}_model.pt  (EMA weights if warmed up),
  prior-sample bank_size paths in R~ space, invert with the SAME frozen
...
OUT_DIR = os.path.join(LS4_DIR, "path_shadowing")
...
def load_gen_model(seed, device):
    """Rebuild the generation model (EMA if present) from the saved checkpoint."""
    ckpt = torch.load(
        os.path.join(LS4_DIR, "weights", f"seed_{seed}_model.pt"),
        map_location=device,
    )
    ...
    if "ema_model" in ckpt:
        ...
        ema = torch.optim.swa_utils.AveragedModel(
            model, avg_fn=lambda a, p, n: lamb * a + (1 - lamb) * p
        )
        ema.load_state_dict(ckpt["ema_model"])
        gen_model = ema.module
    else:
        model.load_state_dict(ckpt["model"])
        gen_model = model
    gen_model.eval()
    gen_model.setup_rnn()
    return gen_model, float(ckpt["sbts_sigma"]), float(ckpt["x_mu"]), float(ckpt["x_sd"])


def build_bank(seed, bank_size, gen_batch, device):
    """Prior-sample `bank_size` LS4 paths, invert with frozen (sigma,x_mu,x_sd)."""
    ...
    with torch.no_grad():
        while done < bank_size:
            ...
            g = gen_model.generate(b, SEQ_LEN, device=device)     # (b,T,1) R~ space
            chunks.append(g.detach().cpu().numpy().astype(np.float32))
            done += b
            if done % (gen_batch * 20) == 0 or done == bank_size:
                el = time.time() - t0
                rate = done / max(el, 1e-9)
                eta = (bank_size - done) / max(rate, 1e-9)
                print(f"  [seed {seed} bank] {done}/{bank_size} "
                      f"{rate:.0f} paths/s ETA {eta:.0f}s", flush=True)
    gen = np.concatenate(chunks, axis=0).astype(np.float64)       # (bank,T,1)
    # inverse: destandardize -> drop dummy col -> undo SBTS scale -> cum-exp @ S0
    X_sbts = gen[:, :, 0] * x_sd + x_mu
    R_tilde = X_sbts[:, 1:]
...
def main():
    ...
    if keep_ens0 is not None:
        ...
        fig.suptitle(f"PS-MC shadowed forecasts — LS4+logret, bank={args.bank_size:,}, K={K}",
                     fontsize=10)
    ...
    fig.suptitle(f"CRPS per forecast step — LS4+logret on Heston ({len(seeds)} seed(s))",
                 fontsize=11)
