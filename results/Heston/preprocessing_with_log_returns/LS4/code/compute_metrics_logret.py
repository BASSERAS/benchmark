"""
Parallel metrics runner for the LS4 log-return-preprocessing experiment.
...
generated paths under results/Heston/preprocessing_with_log_returns/LS4/.
...
to the main-benchmark LS4 results.
...
C.GENERATED_DIR = os.path.join(LS4_DIR, "generated_paths")
C.RESULTS_DIR = LS4_DIR
C.PLOTS_DIR = os.path.join(LS4_DIR, "plots")
...
def load_generated(seed):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}. Run train_ls4_logret.py --seed {seed} first.")
    return np.load(path)
