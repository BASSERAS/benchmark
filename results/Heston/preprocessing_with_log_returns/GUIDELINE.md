...
| E16b | How does RMSE aggregate across queries? | **`RMSE = mean_q(√(mean_u se_{q,u}))`** — root *inside*, then average over queries (the average per-query root error). Implemented in `path_shadowing_pdf.py::_agg` (`np.sqrt(per).mean()`) and mirrored in `_boot_ci` and in the `terminal_rmse` diagnostic (`np.abs(...).mean()`). **Fixed 2026-07-30; this reversed the earlier `√(mean_q mean_u se)` choice.** Two caveats: (a) by Jensen `mean(√·) ≤ √(mean ·)`, so every RMSE cell dropped (e.g. CSDI cum 0.0493 → 0.0414) — old and new numbers must never be mixed in one table; (b) for the **scalar rv** quantity this reduces **exactly to MAE** — so the rv row is **displayed as `MAE ↓`, not `RMSE ↓`** (renamed 2026-07-31). cum/step keep `RMSE ↓`: there H=32 and the inner term is a real per-path horizon-RMS. The rename is **display-only** — the JSON key stays `rmse` everywhere, because renaming it would mean editing the untouched reference `path_shadowing_pdf.py::METRIC_MAP` (5 copies) and re-running the whole 5-bank × 5-method sweep for zero numerical gain. Single source: `build_cross_tables.PS_METRIC_LABEL`, read by both the README renderer and `report/build_report.py`. | Matches the path-shadowing reproducibility report (Tables 1–5), which is the reference the repo's PS tables are checked against cell-by-cell. **No exceptions — every row uses this convention.** `forecaster/pdf_bridge.py` used to pin Chronos-2 / TimesFM to the *old* `√(mean_q se_q)` via a local `_metrics_frozen_rmse()`, so a re-run would reproduce the report's never-recomputed forecaster column. **Removed 2026-07-31:** the bridge now calls `P.metrics_with_ci` directly, so forecasters, generators, the Heston oracle and the RW floor share one byte-identical code path. That exception was not benign — `_ps_winner_idx` ranks *all* `PS_MODELS` including the forecasters, so the Winner column was ranking forecaster cells inflated by a measured 18.0 % (cum) / 5.6 % (step) / 5.0 % (rv) against un-inflated generator ones, while the README footnote simultaneously told readers those cells were not comparable. (Note the rv inflation is 5 %, **not** the generators' 28 % — the ratio tracks per-path error dispersion, which differs between retrieval and direct forecasting, so it must never be used as a substitute for recomputing.) The report's published forecaster RMSE cells are now **superseded, not reproduced** — a re-run will not match them, by design. |
