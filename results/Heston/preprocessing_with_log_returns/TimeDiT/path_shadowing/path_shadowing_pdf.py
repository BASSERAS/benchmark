"""
Path-Shadowing — FULL PDF protocol evaluator (arXiv:2308.01486) for TimeDiT + SBTS
...
methods/TimeDiT/path_shadowing/ — see GUIDELINE.md M7). It is a verbatim clone of
the LS4 PDF evaluator: the metric is method-agnostic (it reads a persisted 1M
price bank from disk), only the bank content differs (TimeDiT-generated here).
...
def eval_bank(qlogS, q_quant, q_prefix_feat, sqrtw, mu, sd, boot_idx,
    ...
    oracle). If None, the single shared 1M TimeDiT bank is mmap-loaded from disk.
    All banks are sliced as nested prefixes B[:bs] and standardized with the
    SAME frozen mu/sd, so TimeDiT / oracle numbers are directly comparable."""
    if bank is None:
...
# is the true DGP. TimeDiT's gap to the oracle = generator law-mismatch; the oracle's own
...
ORACLE_SEED = 777                # independent of test seed and of the TimeDiT bank seed
...
def make_plots(summary):
    ...
    for qn, c in (("cum", "C0"), ("step", "C1"), ("rv", "C2")):
        ...
        ax.plot(bss, m, marker="o", label=f"{qn} (TimeDiT)", color=c)
    ...
    ax.set_title("TimeDiT+logret vs Heston-oracle Path-Shadowing CRPS (one 1M bank, 95% CI)")
