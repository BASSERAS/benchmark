"""
Parallel metrics runner for the TimeDiT log-return-preprocessing experiment.
...
generated paths under results/Heston/preprocessing_with_log_returns/TimeDiT/.
...
to the main-benchmark TimeDiT results.
...
C.GENERATED_DIR = os.path.join(TIMEDIT_DIR, "generated_paths")
C.RESULTS_DIR = TIMEDIT_DIR
C.PLOTS_DIR = os.path.join(TIMEDIT_DIR, "plots")
...
def load_generated(seed):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}. Run train_timedit_logret.py --seed {seed} first.")
    return np.load(path)
