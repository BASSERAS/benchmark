"""
Generate + PERSIST the 1,000,000-path scenario bank for each CSDI+logret seed.
...
this script materialises it once. Bank RNG = manual_seed(1000+seed), identical
to path_shadowing_mc.build_bank, so banks are reproducible and seed-distinct.

Reuses build_bank() from path_shadowing_mc.py verbatim (CSDI unconditional
diffusion sampling + frozen-sigma SBTS inverse). Nothing under methods/ is touched.

Usage (one seed per process, pin cores, pick GPU):
  CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=8 taskset -c 0-7 \
      python gen_banks.py --seed 0
