"""
Parallel metrics runner for the CSDI log-return-preprocessing experiment.
...
generated paths under results/Heston/preprocessing_with_log_returns/CSDI/.
...
to the main-benchmark CSDI results.
...
C.GENERATED_DIR = os.path.join(CSDI_DIR, "generated_paths")
C.RESULTS_DIR = CSDI_DIR
C.PLOTS_DIR = os.path.join(CSDI_DIR, "plots")
...
def load_generated(seed):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Missing: {path}. Run train_csdi_logret.py --seed {seed} first.")
    return np.load(path)
