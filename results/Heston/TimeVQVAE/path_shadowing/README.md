# Path Shadowing MC — TimeVQVAE on Heston
...
> Diffusion-TS, FourierFlow and TimeGAN pipelines
> ([`../../DiffusionTS/path_shadowing/README.md`](../../DiffusionTS/path_shadowing/README.md)).
> Only the generated pool differs (TimeVQVAE instead of the others).
...
|--------|---------|---------|----------------------|-------------------|
| **CRPS** | H=32 | **2.771 ± 0.015** | 2.770 ± 0.015 | 3.732 |
| MAE    | H=32 | 3.701 ± 0.003 | 3.701 ± 0.003 | 3.732 |
| RMSE   | H=32 | 5.066 ± 0.006 | 5.066 ± 0.006 | 5.068 |
| **CRPS** | H=64 | **3.889 ± 0.017** | 3.889 ± 0.017 | 5.301 |
| MAE    | H=64 | 5.227 ± 0.007 | 5.227 ± 0.007 | 5.301 |
| RMSE   | H=64 | 7.159 ± 0.010 | 7.159 ± 0.010 | 7.181 |

**PS-MC beats the naive RW on CRPS** at both horizons by a wide margin
(2.77 < 3.73 at H=32; 3.89 < 5.30 at H=64). TimeVQVAE joins Diffusion-TS as one
of the few methods whose generated pool cleanly clears the random-walk baseline —
its tokenised time-frequency samples supply informative, well-calibrated
nearest-neighbour futures on Heston (Diffusion-TS is marginally lower at H=32,
2.72 vs 2.77, but the two are the only pools that beat RW at *both* horizons).
...
|--------|--------|--------|--------|--------|
| 2.770  | 2.777  | 2.752  | 2.760  | 2.794  |

The cross-seed spread is very tight (2.752–2.794, std 0.015) — every TimeVQVAE
training seed yields a pool of near-identical forecasting quality, and all five
...
| Pool | TimeVQVAE generated paths per seed (8 192 paths) |
...
| η̃ | Adaptive: median(dist) / median(‖z‖) ≈ 9.8 (9.62–9.87 across seeds) |
...
Blue solid = real prefix (0–63). Blue dashed = real future. Red fan = K=77 retrieved TimeVQVAE futures (anchored). Bold red = ensemble mean.
...
python run_eval.py --method TimeVQVAE   # 5 seeds → results/Heston/TimeVQVAE/path_shadowing/
```
