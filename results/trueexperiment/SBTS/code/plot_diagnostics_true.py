...
Panel-for-panel identical to `results/HestonMultiAsset/SBTS/code/
plot_diagnostics_multiasset.py`, so the two READMEs display the same figure in
the same slot and a reader can put the synthetic and the real result side by
side without re-learning the layout.
...
-----
    /home/tbasseras/gpu-venv/bin/python \
        results/trueexperiment/SBTS/code/plot_diagnostics_true.py \
...
MA = os.path.dirname(SBTS)                         # .../trueexperiment
...
def main():
    ...
    # would silently label another method's figure over SBTS's paths.
    ap.add_argument("--method", default=os.path.basename(SBTS))
