...
`results/trueexperiment/CSDI/code/plot_diagnostics_true.py` apart from two
comments naming the tree; keep it that way, because a panel that drifts between
two method trees turns a comparison into a category error.

(The CSDI copy asserts it is verbatim from the SBTS copy. It is not, quite: SBTS
still calls the path constant `SBTS` where CSDI calls it `METHOD_DIR`. The
plotting code itself is identical under an AST comparison, so the figures are
comparable -- but the claim was checked rather than inherited, and it is
recorded here because a "verbatim copy" note that nobody re-verifies is exactly
how two trees drift apart.)
...
-----
    /home/tbasseras/gpu-venv/bin/python \
        results/trueexperiment/Deep-MKV-TS/code/plot_diagnostics_true.py \
