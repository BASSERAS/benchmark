...
# Derived, not hardcoded. The scored set differs per variant -- the 5-seed
# campaigns kept [0, 2, 4, 5, 6] while the quarter-budget run kept [0, 4, 6]
# after two further divergences -- and a stale literal here either plots seeds
# this run never scored or dies on the shorter set. metrics_summary.csv holds
# exactly the set the README tabulates, so the figure and the table cannot
# disagree about which seeds are being reported.
def _scored_seeds():
    path = os.path.join(METHOD_DIR, "metrics_summary.csv")
    with open(path, newline="") as fh:
        header = next(csv.reader(fh))
    ids = sorted(int(c.split("_", 1)[1]) for c in header if c.startswith("seed_"))
    if not ids:
        raise SystemExit(f"no seed_* columns in {path}; refusing to guess")
    return ids
