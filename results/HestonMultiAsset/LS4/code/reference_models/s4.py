...
class SSKernelNPLR(OptimModule):
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        if has_cauchy_extension and z.dtype == torch.cfloat and not self.keops:
            ...
            # FIX (pure-PyTorch fallback): the CUDA/keops cauchy sum over
            # conjugate pole pairs internally, but cauchy_naive does not. Feeding
            # only the N poles here yields HALF the resolvent, so the conv kernel
            # disagrees with the step-mode recurrence (conv-mode != step-mode) and
            # free-running generation drifts while reconstruction stays perfect.
            # Expanding v and w with their conjugates (via _conj) makes the naive
            # fallback numerically identical to the keops path (verified to ~1e-6).
            r = cauchy_naive(_conj(v), z, _conj(w))
