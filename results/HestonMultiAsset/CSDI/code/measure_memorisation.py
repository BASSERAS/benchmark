...
───────────────────────
Nearest-neighbour memorisation diagnostic for the d = 8 CSDI output.
...
extreme failure. Their absence does NOT clear the method, and for CSDI it is close
to uninformative: a sample is the endpoint of 50 stochastic reverse-diffusion
steps driven by fresh Gaussian noise, so bitwise equality with a training path has
probability zero by construction. A zero duplicate count here must not be read as
evidence of novelty; the RATIO is the number that matters.
...
*default* failure mode, while CSDI never touches a training path at generation
time and can only memorise by having encoded the training set into its weights.
On capacity grounds that is a stretch -- 413 057 parameters against 8192*252*8
~ 16.5M training values -- but "implausible" is an argument, not a measurement,
which is why this script exists. The same low ratio would mean different things
for the two methods; the same diagnostic is still the right way to detect it.

Usage
-----
    /home/tbasseras/gpu-venv/bin/python \
        results/HestonMultiAsset/CSDI/code/measure_memorisation.py
...
def main():
    ...
    out = {
        ...
        "per_seed": per_seed,
        "note": "Exact duplicates being 0 does NOT clear the method, and for CSDI it is "
                "close to uninformative: a sample is the endpoint of 50 stochastic "
                "reverse-diffusion steps driven by fresh Gaussian noise, so bitwise "
                "equality with a training path has probability zero by construction. "
                "The ratio is the number that matters. Estimator byte-identical to "
                "SBTS/code/measure_memorisation.py and LS4/code/measure_memorisation.py "
                "so the three columns are directly comparable; SBTS d = 8 scores "
