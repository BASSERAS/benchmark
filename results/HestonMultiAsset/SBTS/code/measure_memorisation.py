...
───────────────────────
Nearest-neighbour memorisation diagnostic for the d = 8 SBTS output.
...
extreme failure. Their absence does NOT clear the method -- SBTS interpolates
between training paths, so it produces near-copies rather than copies.
...
def main():
    ...
    out = {
        ...
        "per_seed": per_seed,
        "note": "Exact duplicates being 0 does NOT clear the method: SBTS interpolates "
                "between training paths, so it produces near-copies rather than copies. "
                "The ratio is the number that matters. The committed d = 1 SBTS entry "
                "scores 0.158 on this same diagnostic, so a low ratio is a property of "
                "SBTS at K=20, not of the d = 8 dataset.",
    }
