...
───────────────────────
Nearest-neighbour memorisation diagnostic for the d = 8 Deep-MKV-TS output.
...
extreme failure. Their absence does NOT clear the method, and for Deep-MKV-TS it is
close to uninformative: a sample is a 251-step Euler-Maruyama rollout of
...
Note the asymmetry with SBTS, worth keeping in view when comparing the columns:
SBTS is an explicit kernel average over training paths, so memorisation is its
*default* failure mode. Deep-MKV-TS never touches a training path at generation
time -- ``model.sample`` only rolls the fitted drift and control forward from
``x0 = log(100)``. But it is not in the same position as a pure likelihood model
either, and the honest statement of the risk is specific to Algorithm 1: during
TRAINING the ridge Z-proxy regresses the adjoint moments onto ``Phi^ref``, a
flattened prefix of the sampled path, and the training bank enters through the
path-functional discrepancy. Memorisation would therefore have to arrive through
the fitted control network's weights, whose exact count is recorded as
``n_parameters`` in ``weights/seed_0_config.json`` and is small against the
8192*252*8 ~ 16.5M training values. That is a capacity argument, and
"implausible" is an argument, not a measurement -- which is why this script exists.
The same low ratio would mean different things for the two methods; the same
diagnostic is still the right way to detect it.

Usage
-----
    /home/tbasseras/gpu-venv/bin/python \
        results/HestonMultiAsset/Deep-MKV-TS/code/measure_memorisation.py
...
def main():
    ...
    out = {
        ...
        "per_seed": per_seed,
        "note": "Exact duplicates being 0 does NOT clear the method, and for Deep-MKV-TS "
                "it is close to uninformative: a sample is a 251-step Euler-Maruyama "
