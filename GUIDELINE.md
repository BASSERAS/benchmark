...
├── <Method>_<Venue>.pdf  # the reference paper PDF (committed; see .gitignore negation below)
...
**The reference paper PDF is committed**, exactly one canonical PDF per method lives at
`methods/<Method>/paper_reimplementation/<Method>_<Venue>.pdf`. All other `*.pdf` are git-ignored; the
paper PDFs are kept via the `.gitignore` negation `!methods/**/paper_reimplementation/*.pdf` (placed
after the global `*.pdf` ignore). The root README "## Methods" catalog links each PDF, the paper (correct
canonical URL), and the original code, grouped by generative family in `render_tables.py` FAMILIES order
(GAN → Diffusion → VAE → Schrödinger Bridge → Fourier Flow) with a `colspan` category separator row.
...
Per the §5.0 split, the loaders in `compute_all.py` are pinned to the **test** (seed 1) and **disc**
(seed 2) files, NOT the bare `heston_S_8192x128.npy` (that seed-0 draw is the generators' training set and
is never scored):

| Variable | Loader | Path | Shape | Scale | Used for |
|----------|--------|------|-------|-------|----------|
| Real prices (test) | `load_real()` | `dataset/heston_S_test_8192x128.npy` | (8192, 128) | original (~100) | A1-A17, A20-A34, all B curves, all plots, PS-MC query prefixes |
| Real variance (test) | `load_real()` | `dataset/heston_v_test_8192x128.npy` | (8192, 128) | original (~0.04) | Heston-spec / vol metrics |
| Real prices (disc) | `load_disc()` | `dataset/heston_S_disc_8192x128.npy` | (8192, 128) | original (~100) | **A18 discriminative + A19 predictive-TSTR ONLY** (the "real" class for the learned judges) |
| Fake prices | `load_fake()` | `methods/<Method>/generated_paths/seed_{i}/generated_paths_8192x128.npy` | (8192, 128) | original | every metric (the method's own generated draw i) |

`compute_all.py` infers the fake path from `--method` and `--dataset` automatically; the test/disc real
files are fixed by the loaders above.
...
Both top-level READMEs are **method-comparison** documents: the A / B / PS-MC tables are **family-grouped
HTML tables** (one leaf column per method, grouped under a colspan super-column per family), NOT the old
`ID | Metric | Category | Dir | Method…` markdown layout. The two READMEs must stay byte-identical in
every comparison table and in the stylised-curves section; only surrounding prose framing may differ.

> **Single deterministic source.** All three comparison tables AND the family headers of the stylised-curves
> section are generated by `metrics/render_tables.py`, `--which A|B|PS` prints the HTML comparison tables
> (+ a win-count comment), `--method <M>` prints one method's markdown. Every value uses its `fmt()`
> (4 sig-figs / sci-notation), so the two READMEs match byte-for-byte. **Never hand-type a table cell or a
> family/method header, regenerate from `render_tables.py` and paste.** The `FAMILIES` list in that file
> (see §15.4.1) is the authoritative, deterministic family→method mapping; column order, colspan grouping,
> and stylised-curve `###`/`####` headers all derive from it.

#### 15.4.1 Deterministic family / category assignment (SOURCE OF TRUTH)

The family of every method, and hence its comparison-table super-column AND its stylised-curves `### <Family>`
group, is fixed **verbatim** by `FAMILIES` in `metrics/render_tables.py` (currently lines 32-41). Reproduce
it exactly; do not invent a new family or re-order:
...
METHOD_DIRS  = [d for _, ms in FAMILIES for d, _ in ms]   # on-disk folder names
METHOD_NAMES = [n for _, ms in FAMILIES for _, n in ms]   # human display names
```

Each entry is `(family_name, [(disk_dir, display_name), …])`. **`disk_dir` ≠ `display_name` for two
methods**, mind this everywhere:

| Family | `disk_dir` (folder under `methods/`, `results/Heston/`) | `display_name` (table + `####` header) |
|--------|--------------------------------------------------------|----------------------------------------|
| GAN | `TimeGAN` | TimeGAN |
...
| Diffusion | `CSDI` | CSDI |
| Diffusion | `TimeMoDE` | TimeMoDE |
| VAE | `TimeVAE` | TimeVAE |
| VAE | `TimeVQVAE` | TimeVQVAE |
| VAE | `LS4` | LS4 |
| Schrödinger Bridge | `SBTS` | SBTS |
| Fourier Flow | `FourierFlow` | **Fourier Flow** |

**To add a method to a family:** append `(disk_dir, display_name)` to that family's list (or add a new
`(family_name, [...])` tuple if it starts a new family). Everything downstream, colspan width, leaf-column
order, win counts, and the stylised-curves group, follows automatically from `render_tables.py`.

#### 15.4.2 Section order (mandatory)

1. **Header**, project one-liner, dataset (Heston, 8192×128), list of methods benchmarked.
2. **A1-A34 comparison table** (`render_tables.py --which A`), family-grouped HTML: a colspan
   super-column per family (GAN, Diffusion, VAE, Schrödinger Bridge, Fourier Flow) over one leaf column per
   method, then `RW baseline`, `Perfect`, `Winner`. Rows grouped by category in the mandatory order
   (Fat Tail → Distribution → Adversarial → Predictive → Temporal → Vol → Heston Spec) with a category
   separator row before each group; A18/A19 split into GRU + MLP sublines. **Winner** cell: ↓ smaller wins;
   ↑ larger wins; A28 Kurtosis Ratio closest-to-1.0 wins. Report the overall win count (denominator **36**,
   counting the A18/A19 GRU+MLP sublines).
3. **B Curve-Shape comparison table** (`render_tables.py --which B`), same family grouping, winner by
   **MSE**, trailing `Perfect` column (non-zero, mean ± std). Three sublines per plot: **MSE** = mean-of-3
   (funct+der+sec_der), **% err / NRMSE = funct-only for every plot** (§8 Section 3). Report B win count
   (`X/6` MSE).
4. **Path Shadowing MC comparison** (`render_tables.py --which PS`), same family grouping; rows
   `PS-MC CRPS H=32 ↓` and `PS-MC CRPS H=64 ↓`; trailing `RW baseline`, **`Perfect`**, `Winner` columns.
   The `Perfect` column is the independent-draw PS-MC floor (`perfect_floor_psmc()` reads
   `methods/perfect_recovery/results/path_shadowing/summary.json`, keys `h32_CRPS_uniform` /
   `h64_CRPS_uniform`, shown mean ± std), a genuine non-zero finite-sample floor, identical across methods.
5. **Stylised curves**, see §15.4.3.
6. **Interpretation bullets**, 3-6 bullets referencing metrics by their IDs (e.g. A20 cov error, A18
   GRU/MLP, A33 Teacher-Sigma Corr).

#### 15.4.3 Stylised-curves section layout (deterministic, identical in both READMEs)

After a short intro paragraph, one `### <Family>` header per family **in `FAMILIES` order**, then under it
one `#### <display_name>` + diagnostic image per method **in that family's order**, then a `---` horizontal
rule **between every family** (after the last method of each family). Concretely:

```markdown
### <Family>

#### <display_name>
![<display_name> diagnostics](<PREFIX>Heston/<disk_dir>/plots/heston_diagnostics.png)

#### <next display_name in family>
![…](<PREFIX>Heston/<disk_dir>/plots/heston_diagnostics.png)

---
```

- `<PREFIX>` is the **only** difference between the two files: root `README.md` uses `results/`
  (i.e. `results/Heston/<disk_dir>/…`); `results/README.md` uses the empty prefix
  (i.e. `Heston/<disk_dir>/…`). Everything else, header text, order, `---` placement, is identical.
- Families and methods follow §15.4.1 exactly: `### GAN` (TimeGAN, COSCI-GAN) → `---` → `### Diffusion`
  (Diffusion-TS, CSDI) → `---` → `### VAE` (TimeVAE, TimeVQVAE, LS4) → `---` → `### Schrödinger Bridge`
  (SBTS) → `---` → `### Fourier Flow` (Fourier Flow) → `---`.
...
- 2026-07-24: **Root README "## Methods" catalog → family-grouped HTML + committed paper PDFs.** The methods catalog in the root `README.md` was reorganised from a flat 10-row markdown table into a family-grouped HTML `<table>` (same taxonomy as the results tables: GAN → Diffusion → VAE → Schrödinger Bridge → Fourier Flow, `render_tables.py` FAMILIES order), each family introduced by a full-width `<tr><td colspan="7">` category separator row. Every **paper link was corrected** to its canonical URL per the authors' own record (TimeGAN → NeurIPS proceedings hash page; COSCI-GAN → OpenReview `RP1CtZhEmR`; GT-GAN → OpenReview `ez6VHWvuXEx`; FourierFlow → `iclr.cc/virtual/2021/poster/2750`; the four wrong/placeholder links removed) and the paper **title** made verbatim. Added a **PDF column** linking the committed reference paper for each method. **One canonical paper PDF is now committed per method** at `methods/<Method>/paper_reimplementation/<Method>_<Venue>.pdf` (10 files, 48 MB total, largest 12 MB, all < 50 MB), kept by the new `.gitignore` negation `!methods/**/paper_reimplementation/*.pdf` after the global `*.pdf` ignore (verified via `git check-ignore`). `methods/GT-GAN/code/README.md` PDF reference updated `GT-GAN_NeurIPS2022_2210.02040v3.pdf` → `GT-GAN_NeurIPS2022.pdf` (redundant 44 MB arXiv copy deleted, user's 2.6 MB OpenReview version kept). Documented in §3.0 (folder tree + PDF-commit rule). No metric value, formula, or results-table changed, catalog + asset change only.

- 2026-07-28: **TimeMoDE added as the 3rd Diffusion-family generator (10th method overall).** TimeMoDE (arXiv:2606.15172, "Towards a Unified Generative Model for Scarce Time Series with Domain Experts", Yao/Zheng/Zuo/Zhang, ICML 2026), a DiT (Diffusion Transformer, hidden 256 / depth 6 / heads 4) with a **Mixture of Domain Experts** (K=8 experts, top-2 routing + 1 shared expert E₀, 53.91 M params, ε-prediction DDPM, T=250), composite loss `simple + 0.01·proto + 0.01·aux`. Appended `("TimeMoDE", "TimeMoDE")` to the Diffusion tuple in **`metrics/render_tables.py` `FAMILIES`** (the single source of truth) and synced this GUIDELINE §15.4.1 codeblock + `disk_dir`/`display_name` table (`disk_dir` = `display_name` = `TimeMoDE`, no aliasing). All three comparison tables (A/B/PS) in both root `README.md` and `results/README.md` were re-rendered through `render_tables.py`, one new column, recomputed Winner/win-counts (TimeMoDE wins **0** of 36 A / 7 B / 2 PS rows, so no prior winner changed). Added the stylised-curves `#### TimeMoDE` entry, the methods catalog row, and a full honest methods-description block (weakest of the diffusion family on Heston: A18 GRU 0.3950, A1 kurtosis 20.10, grid_tvd 35.58 % ± 22.96 %, PS-MC 3.196/4.601 beats the RW baseline but trails all other diffusion; ~5.6 h/seed train, ~26.6 min gen). SLC "From Scratch" reproduction GATE (Context-FID + Discriminative) passed and was user-validated before the 5 Heston seeds. Weights (413 MB/seed, 2.1 GB total > GitHub's 100 MB limit) gitignored via `methods/TimeMoDE/weights/*.pt` (matching the Chronos2/TimesFM convention); per-seed `weights/seed_*_config.json` kept tracked. No metric formula changed, a new-method addition routed entirely through `FAMILIES` + `render_tables.py`.

- 2026-07-28: **Repo-wide typographic dash removal from all README/markdown prose + `render_tables.py` made dash-free.** The user asked to remove en/em dashes and spaced-hyphen dashes from every README ("remove en-em dashed and - from all read me"). Applied a surgical rule set across all 70 tracked non-vendored `.md` files (2233 prose lines changed): em dash `—` and spaced en dash ` – ` and spaced hyphen ` - ` collapse to `, `; a bare range en dash `–` becomes a plain hyphen `-`. **Protected (left untouched):** fenced code blocks, inline `` `code` ``, `$...$`/`$$...$$` LaTeX math (subtraction operators preserved), markdown links, bare URLs, and table-separator rows, so method names (`GT-GAN`, `COSCI-GAN`, `Diffusion-TS`, `Fourier Flow`), CLI flags, ranges like `A1-A34`, and math like `X_{t+1} - X_t` are unchanged; em dashes that survive inside fenced shell-comment/dir-tree examples are intentional. Because the A/B/PS comparison tables are generated, **`metrics/render_tables.py` was made dash-free first** so the single source of truth stays consistent: category labels now render `<b>Fat Tail</b>` (was the illustrative `— Fat Tail —`), the `fmt()` no-value / no-winner placeholder is now `-` (was `—`), and the B curve labels read `lags 1-20` (was `lags 1–20`). All three tables were re-rendered and re-embedded into both root `README.md` and `results/README.md` and re-verified byte-identical to each other and to `render_tables.py` output. The illustrative markdown table skeletons inside this GUIDELINE's fenced examples keep `— <Category> —` as non-normative pedagogy; the authoritative format is the dash-free HTML emitted by `render_tables.py` (§15.4.1). No metric value or formula changed, a typography + generator-output change only.
