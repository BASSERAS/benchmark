"""
Run Path Shadowing Monte-Carlo evaluation for all 5 TimeGAN seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/TimeGAN/path_shadowing/

Generates figures:
  results/Heston/TimeGAN/path_shadowing/plots/ps_mc_example.png
  results/Heston/TimeGAN/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/TimeGAN/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/TimeGAN/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    fig.suptitle("CRPS per forecast step — TimeGAN on Heston (5 seeds)", fontsize=11)
