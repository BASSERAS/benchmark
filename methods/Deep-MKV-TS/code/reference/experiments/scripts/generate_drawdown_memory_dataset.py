...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--split-index", type=int, default=64)
    parser.add_argument("--history-cutoff", type=int, default=40)
...
def main() -> None:
    ...
    if not 1 <= int(args.split_index) < int(args.sequence_length) - 1:
        raise ValueError("split_index must be inside the path")
    if not 1 <= int(args.history_cutoff) <= int(args.split_index):
        raise ValueError("history_cutoff must be in [1, split_index]")
    if not 1 <= int(args.recent_window) <= int(args.split_index):
        raise ValueError("recent_window must be in [1, split_index]")
    if not 1 <= int(args.future_horizon) <= int(args.sequence_length) - 1 - int(args.split_index):
    ...
    configuration = {
        ...
        "history_cutoff": int(args.history_cutoff),
