"""
Run Path Shadowing Monte-Carlo evaluation for all 5 Deep-MKV-TS seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/Deep-MKV-TS/path_shadowing/
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/Deep-MKV-TS/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/Deep-MKV-TS/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/Deep-MKV-TS/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — Deep-MKV-TS on Heston (5 seeds)", fontsize=11)
