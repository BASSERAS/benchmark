"""
Run Path Shadowing Monte-Carlo evaluation for all 5 CSDI seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/CSDI/path_shadowing/

Generates figures:
  results/Heston/CSDI/path_shadowing/plots/ps_mc_example.png
  results/Heston/CSDI/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/CSDI/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/CSDI/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/CSDI/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — CSDI on Heston (5 seeds)", fontsize=11)
