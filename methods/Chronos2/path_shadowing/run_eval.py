"""
Run Path Shadowing Monte-Carlo evaluation for all 5 Chronos2 seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/Chronos2/path_shadowing/

Generates figures:
  results/Heston/Chronos2/path_shadowing/plots/ps_mc_example.png
  results/Heston/Chronos2/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/Chronos2/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/Chronos2/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/Chronos2/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — Chronos2 on Heston (5 seeds)", fontsize=11)
