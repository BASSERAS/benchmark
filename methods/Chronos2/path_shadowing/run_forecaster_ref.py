"""
Chronos-2 as a FORECASTER REFERENCE on the path-shadowing task.

Role in the benchmark
---------------------
Chronos-2 is a *conditional* probabilistic forecaster, not an unconditional
generator (its retired generator experiment lives in methods/Chronos2/old/).
Here we use it for what it is actually built for: given the 64-step real prefix
of each held-out Heston test path, forecast the next 64 steps directly with a
single `predict_quantiles(prediction_length=64)` call.  This yields a "best
...
Two rows are produced:
    * zero_shot : released `amazon/chronos-2` weights, no training (one model).
    * finetune  : the 5 full-finetuned seed checkpoints in ../weights/ ,
                  reported as mean +/- std across the 5 seeds.

Ensemble construction
---------------------
`predict_quantiles` returns 21 predictive quantiles per future step.  We turn
that into a K=77-member ensemble per (series, step) by piecewise-linear
inverse-CDF sampling over the 21 quantile levels, giving an ensemble of shape
(N, K, H).  CRPS is then the exact energy score used by the generators'
PS-MC harness (path_shadowing.crps), so a forecaster row and a generator row
are apples-to-apples.

Forecasting is done in PRICE space (the same scale the fine-tune checkpoints
were trained on); Chronos-2 handles context normalisation internally, and a
single-shot 64-step joint forecast has no autoregressive feedback so it is
stable (unlike the retired generator rollout).

Usage
-----
  CUDA_VISIBLE_DEVICES=0 python run_forecaster_ref.py
Outputs -> results/Heston/Chronos2/  (forecaster_summary.json, per-seed JSON,
...
PS_DIR = os.path.dirname(os.path.abspath(__file__))                # methods/Chronos2/path_shadowing
METHOD_DIR = os.path.dirname(PS_DIR)                               # methods/Chronos2
BENCH_ROOT = os.path.dirname(os.path.dirname(METHOD_DIR))          # benchmark/
REF_SRC = os.path.join(METHOD_DIR, "code", "reference", "src")
WEIGHTS_DIR = os.path.join(METHOD_DIR, "weights")
OUT_DIR = os.path.join(BENCH_ROOT, "results", "Heston", "Chronos2")
...
# --------------------------------------------------------------------------- #
def forecast_ensemble(pipeline, prefixes, horizon, K, rng, batch_size=512):
    """
    prefixes : (N, PREFIX_LEN) price-scale context.
    Returns  : ensemble (N, K, horizon) price-scale.
    """
    q_levels = np.asarray(list(pipeline.quantiles), dtype=np.float64)   # (Q,)
    N = prefixes.shape[0]
    Q = q_levels.shape[0]
    q_all = np.empty((N, horizon, Q), dtype=np.float64)
    for s in range(0, N, batch_size):
        e = min(s + batch_size, N)
        ctx = [torch.tensor(prefixes[i], dtype=torch.float32) for i in range(s, e)]
        quant, _ = pipeline.predict_quantiles(
            ctx, prediction_length=horizon, quantile_levels=q_levels.tolist(),
        )
        for j, qt in enumerate(quant):
            q_all[s + j] = qt.detach().cpu().numpy()               # (horizon, Q)
...
def load_pipeline(device, state_dict_path=None):
    pipe = BaseChronosPipeline.from_pretrained(
        MODEL_ID, device_map=device, torch_dtype=torch.float32)
    if state_dict_path is not None:
        sd = torch.load(state_dict_path, map_location=device, weights_only=False)
        pipe.model.load_state_dict(sd["model"])
...
def main():
    ...
    N, T = X_real.shape
    prefixes = X_real[:, :PREFIX_LEN]                              # (N, 64)
    y_fut = X_real[:, PREFIX_LEN:]                                 # (N, 64)
    print(f"=== Chronos-2 forecaster reference  device={dev_name} ===", flush=True)
    ...
    t0 = time.time()
    pipe = load_pipeline(device, state_dict_path=None)
    rng = np.random.default_rng(0)
    ens_zs = forecast_ensemble(pipe, prefixes, HORIZON, K, rng, batch_size=FC_BATCH)
    ...
    for seed in range(5):
        ...
        pipe = load_pipeline(device, state_dict_path=sd_path)
        rng = np.random.default_rng(seed)
        ens = forecast_ensemble(pipe, prefixes, HORIZON, K, rng, batch_size=FC_BATCH)
    ...
    fig.suptitle(
        f"Chronos-2 forecaster reference — 64-step prefix (blue) + K={K} "
