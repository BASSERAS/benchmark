...
#
# ---------------------------------------------------------------------------
# PROVENANCE (do not edit the metric logic below)
# ---------------------------------------------------------------------------
# Verbatim copy of the official GT-GAN reference metric:
#     methods/GT-GAN/code/reference/metrics/discriminative_metrics.py
# which itself is the TimeGAN post-hoc discriminative score (Yoon et al.,
# NeurIPS'19) as vendored by the GT-GAN authors (Jeon et al., NeurIPS'22).
#
# This is the *paper metric* definition. It is TensorFlow 1.x and requires the
# `train_test_divide / extract_time / batch_generator` helpers from the
# reference `utils.py` (on sys.path via reproduce_stock.py). It will NOT run on
# this CUDA-13 box (TF1 cannot bind the driver) — see ../README.md §"caveat".
# It is committed here so the exact paper protocol is auditable next to the
# results. The committed 5-seed numbers were obtained with the benchmark's
# shared PyTorch TSTR port; this file documents the reference definition.
# ---------------------------------------------------------------------------
