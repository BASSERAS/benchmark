"""
Run Path Shadowing Monte-Carlo evaluation for all 5 TimeVQVAE seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/TimeVQVAE/path_shadowing/

Generates figures:
  results/Heston/TimeVQVAE/path_shadowing/plots/ps_mc_example.png
  results/Heston/TimeVQVAE/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/TimeVQVAE/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/TimeVQVAE/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/TimeVQVAE/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — TimeVQVAE on Heston (5 seeds)", fontsize=11)
