"""
Run Path Shadowing Monte-Carlo evaluation for all 5 FourierFlow seeds in parallel.

Identical to methods/TimeGAN/path_shadowing/run_eval.py — only the method name and
generated-path locations differ (TimeGAN -> FourierFlow). The PS-MC algorithm in
path_shadowing.py is model-agnostic: it consumes only the generated .npy files.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/FourierFlow/path_shadowing/

Generates figures:
  results/Heston/FourierFlow/path_shadowing/plots/ps_mc_example.png
  results/Heston/FourierFlow/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/FourierFlow/path_shadowing")
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(
        BENCH, f"methods/FourierFlow/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/FourierFlow/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — FourierFlow on Heston (5 seeds)", fontsize=11)
