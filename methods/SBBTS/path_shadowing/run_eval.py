"""
Run Path Shadowing Monte-Carlo evaluation for all 5 SBBTS seeds in parallel.

Identical to methods/TimeGAN/path_shadowing/run_eval.py — only two lines differ:
  - OUT_DIR  -> results/Heston/SBBTS/path_shadowing/
  - X_fake   -> methods/SBBTS/generated_paths/seed_{seed}/generated_paths_8192x128.npy
...
Saves per-seed JSON + summary JSON to:
  results/Heston/SBBTS/path_shadowing/
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/SBBTS/path_shadowing")   # ← SBBTS
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(                                         # ← SBBTS
        BENCH, f"methods/SBBTS/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    crps_per_step_u = np.array([r[1] for r in raw])
    crps_per_step_g = np.array([r[2] for r in raw])
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/SBBTS/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle(
        f"Path Shadowing MC — SBBTS — prefix (blue) + K={K} futures (red), seed 0",
        fontsize=10
    )
    ...
    fig.suptitle("CRPS per forecast step — SBBTS on Heston (5 seeds)", fontsize=11)
