...
| SBTS synthetic paths, per seed `i` | `sbts-venv`, `SBTS_NWORK=64` | `run_all.py` (seed `i`) | `dataset/Heston/heston_S_8192x128.npy` (8192,128), loaded at `run_all.py:45,58` and passed to `generate_paths(S_train, ...)`. This is the only input: SBTS never receives the Heston SDE parameters | `methods/SBTS/generated_paths/seed_i/generated_paths_8192x128.npy` |
