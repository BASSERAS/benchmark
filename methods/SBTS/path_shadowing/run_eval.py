"""
Run Path Shadowing Monte-Carlo evaluation for all 5 SBTS seeds in parallel.

Identical to methods/TimeGAN/path_shadowing/run_eval.py — only two lines differ:
  - OUT_DIR  -> results/Heston/SBTS/path_shadowing/
  - X_fake   -> methods/SBTS/generated_paths/seed_{seed}/generated_paths_8192x128.npy
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/SBTS/path_shadowing")   # ← SBTS
...
def eval_seed(seed):
    ...
    X_fake = np.load(os.path.join(                                         # ← SBTS
        BENCH, f"methods/SBTS/generated_paths/seed_{seed}"
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/SBTS/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle(
        f"Path Shadowing MC — SBTS — prefix (blue) + K={K} futures (red), seed 0",
        fontsize=10
    )
    ...
    fig.suptitle("CRPS per forecast step — SBTS on Heston (5 seeds)", fontsize=11)
