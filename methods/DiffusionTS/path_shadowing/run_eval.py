"""
Run Path Shadowing Monte-Carlo evaluation for all 5 DiffusionTS seeds in parallel.
...
Saves per-seed JSON + summary JSON to:
  results/Heston/DiffusionTS/path_shadowing/

Generates figures:
  results/Heston/DiffusionTS/path_shadowing/plots/ps_mc_example.png
  results/Heston/DiffusionTS/path_shadowing/plots/crps_per_step.png
"""
...
OUT_DIR    = os.path.join(BENCH, "results/Heston/DiffusionTS/path_shadowing")
...
if __name__ == "__main__":
    ...
    X_fake0 = np.load(os.path.join(
        BENCH, "methods/DiffusionTS/generated_paths/seed_0/generated_paths_8192x128.npy"
    ))
    ...
    fig.suptitle("CRPS per forecast step — DiffusionTS on Heston (5 seeds)", fontsize=11)
